      ...
      <main className="relative z-10 flex flex-1 flex-col">
        ...
        <section className="mx-auto w-full max-w-6xl px-6 pb-8 pt-16 sm:pt-20">
          <div className="mx-auto flex max-w-3xl flex-col items-center gap-6 text-center">
            <span className="surface inline-flex items-center gap-1.5 rounded-full border border-border bg-card px-3 py-1 text-xs text-muted-foreground">
              <Sparkles className="size-3.5 text-brand" />
              Conversations as graphs
            </span>
            <h1 className="text-balance text-4xl font-semibold tracking-tight sm:text-6xl">
              Chat in graphs, <span className="text-brand">not threads</span>
            </h1>
        ...
        <section id="features" className="mx-auto w-full max-w-6xl scroll-mt-20 px-6 py-20">
          ...
          <div className="mt-12 grid gap-5 md:grid-cols-3">
            {FEATURES.map((f) => (
              <div key={f.title} className="surface flex flex-col gap-4 rounded-xl bg-card p-6">
                <span className="flex size-10 items-center justify-center rounded-lg border border-brand/25 bg-brand/10">
